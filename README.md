...
首次完成下方的环境安装后，使用一个命令启动前端、本地 API 和视觉分析 Worker：

```bash
./start-local.sh
```

然后打开 `http://127.0.0.1:5173`。脚本会复用该端口上已运行的前端，不会悄悄改用其他端口；按 `Ctrl+C` 会停止本次脚本启动的服务。若前端依赖尚未安装，脚本会自动执行 `npm install`；Python 虚拟环境仍需按下方步骤先准备好。
