...
| 3 | 数据契约与持久化 | 基础 | done |
| 4 | 设计系统与界面基础 | 基础 | done |
| 5 | 最小端到端骨架，一段视频到一份结果 | 骨架 | done |
| 6 | 基线可复现与对比 | 切片二 | blocked |
| 7 | 多人员跟踪与风险事件 | 切片三 | in-progress |
| 8 | 证据链与报告 | 切片四 | done |
| 9 | Web 分析流程 | 切片五 | done |
| 10 | 姿态证据驱动的 ErgoAgent | 切片六 | done |
...
- [x] 实现 Web 分析流程：上传、轮询、结果、事件、证据帧和结果产物下载

## 切片六

### 10. 姿态证据驱动的 ErgoAgent · done · Full
...
当前后续未完成事项是：在外部仓库运行时可复现后形成同视频基线指标、用多人回归视频验证 HOG 候选框的遮挡和 ID 交换、补齐视频不可观测的 REBA 负荷/耦合/活动输入，以及补浏览器截图 QA 和比赛交付材料。报告双 artifact、标准表格代理评分、可选多人检测器和结构化证据助手已进入代码，但结论仍是辅助评估，不应描述为专家诊断或完整多人能力。
