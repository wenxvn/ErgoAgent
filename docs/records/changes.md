...
## 2026 年 9 月 4 日，完成标注可见性和分析预览改进

范围：处理标注视频过细、播放体验和分析过程中预览不连续的反馈。

改动：

1. 标注视频按输出分辨率缩放线宽、关键点半径和文字，并使用深色描边、骨架连线和高对比标签；高风险与普通风险使用稳定的颜色区分。
2. 分析预览先缩放到最多 1280px 宽，再以约 8 FPS 从真实 Worker 帧输出原子替换 JPEG；前端以实际帧号缓存键每 0.25 秒轮询，避免读取半写文件，并展示原视频帧率、预览采样率和网页刷新率的差异。
3. 最终视频保留 H.264 `faststart` 输出，前端增加元数据、当前时间和上一帧/下一帧定位；FileResponse 的字节 Range 响应已核验。
4. 修复 `0002_task_progress` 在 `0001` 已由 ORM 创建进度列时的重复列迁移错误，使全新数据库升级幂等。

验证：`.venv/bin/pytest -q` 为 13 passed；前端 `npm run lint`、`npm run build` 通过；4K 样例真实运行生成 284 帧、252 检测帧、3840×2160 H.264 标注视频和 1280×720 预览；视频 Range 请求返回 206。浏览器跨设备截图和长视频持续播放仍需在目标浏览器中补充人工验收。
